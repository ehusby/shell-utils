...
## Script globals
script_file_rel="${BASH_SOURCE[0]}"
script_fname=$(basename "$script_file_rel")
script_dir=$({ cd "$(dirname "$script_file_rel")" || { echo "Failed to access script file directory" >&2; exit; } } && pwd)
script_dir_abs=$({ cd "$(dirname "$script_file_rel")" || { echo "Failed to access script file directory" >&2; exit; } } && pwd -P)
script_file="${script_dir}/${script_fname}"
script_file_abs="${script_dir_abs}/${script_fname}"
