...
EXCLUDE_DNAMES, EXCLUDE_FNAMES = [
    exclude_names if exclude_names != [""] else None
    for exclude_names in (
        EXCLUDE_DNAMES,
        EXCLUDE_FNAMES,
    )
]
EXCLUDE_DPATHS, EXCLUDE_FPATHS = [
    [os.path.abspath(os.path.expanduser(path)) for path in exclude_paths]
    if exclude_paths != [""]
    else None
    for exclude_paths in (EXCLUDE_DPATHS, EXCLUDE_FPATHS)
]

default_src = SRC if SRC is not None and SRC != "" else None
default_dstdir = DSTDIR if DSTDIR is not None and DSTDIR != "" else None
default_flist_srcdir = (
    FLIST_SRCDIR if FLIST_SRCDIR is not None and FLIST_SRCDIR != "" else None
)
...
default_fprefix = (
    FNAME_PREFIX if FNAME_PREFIX is not None and FNAME_PREFIX != "" else None
)
default_dprefix = (
    DNAME_PREFIX if DNAME_PREFIX is not None and DNAME_PREFIX != "" else None
)
default_fcontains = (
    FNAME_CONTAINS if FNAME_CONTAINS is not None and FNAME_CONTAINS != "" else None
)
default_dcontains = (
    DNAME_CONTAINS if DNAME_CONTAINS is not None and DNAME_CONTAINS != "" else None
)
default_fsuffix = (
    FNAME_SUFFIX if FNAME_SUFFIX is not None and FNAME_SUFFIX != "" else None
)
default_dsuffix = (
    DNAME_SUFFIX if DNAME_SUFFIX is not None and DNAME_SUFFIX != "" else None
)
default_freplace = (
    FNAME_REPLACE if FNAME_REPLACE is not None and FNAME_REPLACE != "" else None
)
default_dreplace = (
    DNAME_REPLACE if DNAME_REPLACE is not None and DNAME_REPLACE != "" else None
)
...
def main():
    ...
    parser.add_argument(
        ...
        help=("Create hard links." + " (default)" * default_hardlink),
    )
    ...
    parser.add_argument(
        ...
        help=("Create symbolic links." + " (default)" * default_symlink),
    )
    ...
    parser.add_argument(
        ...
        default=default_src,
        help=(
            "Path to source directory containing files to link to, "
    ...
    parser.add_argument(
        ...
        help=(
            "Path to destination directory where file tree of links will be created. "
            "If `src` is a file/dir list, all linked files/dirs are put directly in this folder."
            + " (default={})".format(default_dstdir)
            * (default_dstdir is not None)
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Depth of recursion, in terms of directory levels below the level of the root. "
    ...
    parser.add_argument(
        "--transplant-tree",
        action="store_true",
        default=default_transplant_tree,
        help=(
            "(Only applies when `src` is a directory path.) "
    ...
    parser.add_argument(
        "--collapse-tree",
        action="store_true",
        default=default_collapse_tree,
        help=(
            "(Only applies when `src` is a directory or a dir list.) "
    ...
    parser.add_argument(
        "--flist-srcdir",
        default=default_flist_srcdir,
        help=(
            "If `src` is a file/dir list, prepend this directory to all "
            "source file entries before processing."
            + " (default={})".format(default_flist_srcdir)
            * (default_flist_srcdir is not None)
        ),
    )
    ...
    parser.add_argument(
        "--flist-glob",
        action="store_true",
        default=default_flist_glob,
        help=(
            "If `src` is a file/dir list, use Unix-style pathname pattern expansion "
    ...
    parser.add_argument(
        ...
        help=(
            "Deliminating character for multiple --[d/f][prefix/contains/suffix] specification. "
    ...
    parser.add_argument(
        "--fprefix",
        default=default_fprefix,
        help=(
            "Only include files with a name that starts with this string. "
            "If `src` is a file list with filenames like qsub_*, syntax can be like "
            "'qsub_>' to link files without the qsub_ prefix."
            + " (default={})".format(default_fprefix)
            * (default_fprefix is not None)
        ),
    )

    parser.add_argument(
        "--fcontains",
        default=default_fcontains,
        help=(
            "Only include files with a name that contains this string. "
            "If `src` is a file list with filenames like *_2m_*, syntax can be like "
            "'_2m_>_2m_{}_8m_' to link additional *_8m_* component files.".format(
                default_delim
            )
            + " (default={})".format(default_fcontains)
            * (default_fcontains is not None)
        ),
    )

    parser.add_argument(
        "--fsuffix",
        default=default_fsuffix,
        help=(
            "Only include files with a name that ends with this string. "
            ...
            "'>*' along with --flist-glob option to link all component files.".format(
                default_delim
            )
    ...
    parser.add_argument(
        "--freplace",
        default=default_freplace,
        help=(
            "Replace all instances of a (set of) substring(s) in filenames, "
            "syntax like 'ss1_orig>ss1_repl{}ss2_orig>ss2_repl' where "
            "replacements are evaluated left to right in overwriting fashion.".format(
                default_delim
            )
    ...
    parser.add_argument(
        "--dprefix",
        default=default_dprefix,
        help=(
            "Only include directories with a name that starts with this string."
    ...
    parser.add_argument(
        "--dcontains",
        default=default_dcontains,
        help=(
            "Only include directories with a name that contains this string."
            + " (default={})".format(default_dcontains)
            * (default_dcontains is not None)
        ),
    )

    parser.add_argument(
        "--dsuffix",
        default=default_dsuffix,
        help=(
            "Only include directories with a name that ends with this string."
    ...
    parser.add_argument(
        "--dreplace",
        default=default_dreplace,
        help=(
            "Replace all instances of a (set of) substring(s) in directory names, "
            "syntax like 'ss1_orig>ss1_repl{}ss2_orig>ss2_repl' where "
            "replacements are evaluated left to right in overwriting fashion. "
            "*Only applies when `src` is a directory or a dir list.*".format(
                default_delim
            )
    ...
    parser.add_argument(
        ...
        help=(
            "If a file already exists at the path where the link is to be created "
    ...
    if system not in system_choices:
        parser.error(
            "Only supported system types are {}, " "but detected '{}'".format(
                system_choices, system
            )
        )
    ...
    FLIST_SRCDIR = (
        os.path.abspath(os.path.expanduser(args.flist_srcdir))
        if args.flist_srcdir is not None
        else None
    )
    DELIM = args.delim
    (
        ...
        s.split(DELIM) if s is not None else None
        for s in (
            args.fprefix,
            args.dprefix,
            args.fcontains,
    ...
    if os.path.isdir(SRC):
        if [
            flist_arg is not None
            for flist_arg in (FLIST_PREFIX, FLIST_CONTAINS, FLIST_SUFFIX)
        ].count(True) > 0:
            parser.error(
                "`src` must be a text file to use extension of "
        ...
        if [
            ...
            parser.error(
                "Only one of (--fprefix, --fcontains, --fsuffix) options "
    ...
    try:
        ...
        print(
            "Python built-in link function is not available "
            "on this system ({}) and/or Python version ({})".format(
        ...
        if TRANSPLANT_TREE:
            ...
            if DNAME_REPLACE is not None:
                for repl_item in DNAME_REPLACE:
                    link_rootdir_name = link_rootdir_name.replace(
                        repl_item[0], repl_item[1]
                    )
...
def link_file(src_file, dst_file):
    if os.path.isfile(dst_file):
        if os.stat(src_file).st_ino == os.stat(dst_file).st_ino:
            ...
            if VERBOSE:
                print(
                    "File already exists, but is not the correct link and will be {}: {}".format(
...
def link_dir(srcdir, dstdir, depth):
    for dirent in os.listdir(srcdir):
        ...
        name_replacements = (
            DNAME_REPLACE if os.path.isdir(src_dirent) else FNAME_REPLACE
        )
        ...
        if os.path.isdir(src_dirent):
            if (
                ...
                and (
                    DNAME_PREFIX is None
                    or True in (dirent.startswith(dp) for dp in DNAME_PREFIX)
                )
                and (
                    DNAME_CONTAINS is None
                    or True in (dc in dirent for dc in DNAME_CONTAINS)
                )
                and (
                    DNAME_SUFFIX is None
                    or True in (dirent.endswith(ds) for ds in DNAME_SUFFIX)
                )
            ):
            ...
            if (
                (EXCLUDE_FNAMES is None or dirent not in EXCLUDE_FNAMES)
                and (EXCLUDE_FPATHS is None or src_dirent not in EXCLUDE_FPATHS)
                and (
                    FNAME_PREFIX is None
                    or True in (dirent.startswith(fp) for fp in FNAME_PREFIX)
                )
                and (
                    FNAME_CONTAINS is None
                    or True in (fc in dirent for fc in FNAME_CONTAINS)
                )
                and (
                    FNAME_SUFFIX is None
                    or True in (dirent.endswith(fs) for fs in FNAME_SUFFIX)
                )
            ):
    ...
    with open(flist, "r") as flist_fp:
        for line_num, line in enumerate(flist_fp):
            ...
            if DELIM in flist_item:
                ...
                if not (
                    txt_dname.startswith("/") or txt_dname[1] == ":"
            ...
            if (
                (EXCLUDE_DNAMES is None or txt_dname not in EXCLUDE_DNAMES)
                and (EXCLUDE_DPATHS is None or txt_dpath not in EXCLUDE_DPATHS)
                and (
                    DNAME_PREFIX is None
                    or True in (txt_dname.startswith(dp) for dp in DNAME_PREFIX)
                )
                and (
                    DNAME_CONTAINS is None
                    or True in (dc in txt_dname for dc in DNAME_CONTAINS)
                )
                and (
                    DNAME_SUFFIX is None
                    or True in (txt_dname.endswith(ds) for ds in DNAME_SUFFIX)
                )
            ):
                pass
            else:
            ...
            if txt_fname == "":
                ...
                if FLIST_GLOB:
                    ...
                    for dir_pattern in src_dirs:
                        src_dirs_glob.extend(
                            [d for d in glob.glob(dir_pattern) if os.path.isdir(d)]
                        )
                    ...
                    if not src_dirs_glob:
                        print(
                            "Glob for directory path pattern '{}' returned 0 matching directories".format(
                                dir_pattern
                            )
                        )
                for d in src_dirs:
                    ...
                    if not os.path.isdir(d):
                        print(
                            "Source file list line {}: "
                            "Missing source directory '{}', skipping".format(
                                line_num + 1, d
                            )
                        )
                        continue
                    if TRANSPLANT_TREE:
                        link_rootdir_name = os.path.basename(
            ...
            if FLIST_PREFIX is not None:
                ...
                if repl_index == -1:
                    print(
                        "Could not find file name prefix '{}' "
                        "in source file list line {}: {}".format(
                            FLIST_PREFIX, line_num + 1, txt_fpath
                        )
                    )
                    continue
                ...
                if FNAME_PREFIX is not None:
                    src_fnames = [
                        link_pref + txt_fname_suff for link_pref in FNAME_PREFIX
                    ]
                else:
                ...
                if repl_index == -1:
                    print(
                        "Could not find file name suffix '{}' "
                        "in source file list line {}: {}".format(
                            FLIST_SUFFIX, line_num + 1, txt_fpath
                        )
                    )
                    continue
                ...
                if FNAME_SUFFIX is not None:
                    src_fnames = [
                        txt_fname_pref + link_suff for link_suff in FNAME_SUFFIX
                    ]
                else:
                ...
                src_fnames = [
                    txt_fname.replace(FLIST_CONTAINS, link_cont)
                    for link_cont in FNAME_CONTAINS
                ]
            ...
            if FLIST_GLOB or EXCLUDE_FPATHS is not None:
                if FLIST_GLOB:
                    src_files = []
                    for file_pattern in [
                        os.path.join(txt_dpath, GLOB_PREFIX + fname + GLOB_SUFFIX)
                        for fname in src_fnames
                    ]:
                        src_files_glob = glob.glob(file_pattern)
                        if not src_files_glob:
                            print(
                                "Glob for file path pattern '{}' returned 0 matching files".format(
                                    file_pattern
                                )
                            )
                        else:
            ...
            src_files = [
                os.path.join(txt_dpath, src_fname)
                for src_fname in src_fnames
                if (
                    (EXCLUDE_FNAMES is None or src_fname not in EXCLUDE_FNAMES)
                    and (
                        FNAME_PREFIX is None
                        or FLIST_PREFIX is not None
                        or True in (src_fname.startswith(fp) for fp in FNAME_PREFIX)
                    )
                    and (
                        FNAME_CONTAINS is None
                        or FLIST_CONTAINS is not None
                        or True in (fc in src_fname for fc in FNAME_CONTAINS)
                    )
                    and (
                        FNAME_SUFFIX is None
                        or FLIST_SUFFIX is not None
                        or True in (src_fname.endswith(fs) for fs in FNAME_SUFFIX)
                    )
                )
            ]
            if len(src_files) == 0:
            ...
            if missing_component:
                print(
                    "Source file list line {}: "
                    "Skipping source file '{}' due to missing component".format(
                        line_num + 1, txt_fpath
                    )
                )
                continue
            ...
            for src_dirent in src_files:
                ...
                if FNAME_REPLACE is not None:
                    for repl_item in FNAME_REPLACE:
                        dst_dirent_name = dst_dirent_name.replace(
                            repl_item[0], repl_item[1]
                        )
                dst_dirent = os.path.join(dstdir, dst_dirent_name)
